...
export default function Leaderboard() {
  ...
  useEffect(() => { fetchResource('leaderboard').then(setEntries).catch((error) => setState({ loading: false, error: error.message })).finally(() => setState((current) => ({ ...current, loading: false }))); }, []);
